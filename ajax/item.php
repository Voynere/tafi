	...
	if (!empty($_REQUEST["add_item"])) {
		if ($_REQUEST["add_item"] == "Y") {
			...
			foreach ($products_in_cart->arResult as $ids) {
				$arProductsInCart[] = $ids["PRODUCT_ID"];
			}

			$resAddItem = CIBlockElement::GetProperty(26, $_REQUEST["item"], array("sort" => "asc"), array());
			while ($obAddItem = $resAddItem->GetNext()) {
				if ($obAddItem["CODE"] == "TO_BASKET_TO" && $obAddItem["VALUE"] != "" && !in_array($obAddItem["VALUE"], $arProductsInCart)) {
					Add2BasketByProductID($obAddItem["VALUE"], 1, array(array("NAME" => "Цвет", "CODE" => "CLR", "VALUE" => "красный"), array("NAME" => "Размер", "VALUE" => "25")));
				}
			}
			...
			/* if (!empty($products_in_cart->arResult)) {
				...
				foreach ($arrValueProp as $valProps) {
					if (!in_array($valProps, $arProductsInCart)) {
						Add2BasketByProductID($valProps, 1, array(array("NAME" => "Цвет", "CODE" => "CLR", "VALUE" => "красный"), array("NAME" => "Размер", "VALUE" => "25")));
					}
				...
				$res = CIBlockElement::GetProperty(26, $_REQUEST["item"], array("sort" => "asc"), array());
				while ($ob = $res->GetNext()) {
					if ($ob["CODE"] == "TO_BASKET_TO" && $ob["VALUE"] != "") {
						$idsArr[] = $_REQUEST["item"];
						$arrValueProp[] = $ob["VALUE"];
					}
				}
				foreach ($arrValueProp as $valProps) {
			...
			//
			//Add2BasketByProductID(1714, 2, array(array("NAME" => "Цвет", "CODE" => "CLR", "VALUE" => "красный"),array("NAME" => "Размер", "VALUE" => "25")));
