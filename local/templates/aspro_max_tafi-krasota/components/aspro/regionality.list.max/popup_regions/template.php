...
<?
if (!function_exists('asproRegionNormalizePath')) {
	function asproRegionNormalizePath($urlOrPath) {
...
if (!function_exists('asproRegionIsSelfLink')) {
	function asproRegionIsSelfLink($link, $curPage) {
		$n1 = asproRegionNormalizePath($link);
		$n2 = asproRegionNormalizePath($curPage);
		return $n1 !== '' && $n1 === $n2;
	}
}
				...
				<div class="favorits">
					...
					<div class="cities">
						<?foreach($arResult['FAVORITS'] as $arItem):?>
							<?$bSelfLinkFav = asproRegionIsSelfLink($arItem['URL'] ?? '', $__curPage);?>
							<div class="item">
								<?if(!$bSelfLinkFav):?>
					...
					<div class="block cities">
						...
						<div class="items_block">
							<?foreach($arResult['REGIONS'] as $key => $arItem):?>
								...
								<?$bSelfLink = asproRegionIsSelfLink($arItem['URL'] ?? '', $__curPage);?>
								<div class="item  <?=($bCurrent ? 'current shown' : '');?> <?=((!$arResult['SECTION_LEVEL1'] && !$arResult['SECTION_LEVEL2']) ? 'shown' : '');?>" data-id="<?=((isset($arItem['IBLOCK_SECTION_ID']) && $arItem['IBLOCK_SECTION_ID']) ? $arItem['IBLOCK_SECTION_ID'] : 0);?>">
									<?if($bCurrent):?>
										...
										<?else:?>
											<span class="name dark_link"><?=$arItem['NAME'];?></span>
										<?endif;?>
									<?else:?>
