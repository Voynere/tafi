...
$__curPage = is_object($APPLICATION) ? (string)$APPLICATION->GetCurPage(false) : '';
if ($__curPage === '') {
	$__curPage = (string)(defined('PAGE_NAME') ? PAGE_NAME : '');
}
$GLOBALS['__curPage'] = $__curPage;

if (!function_exists('asproMenuNormalizePath')) {
	function asproMenuNormalizePath($urlOrPath) {
		...
		// trailing slash for "directory" urls
...
if (!function_exists('asproMenuIsSelfLink')) {
	function asproMenuIsSelfLink($link, $curPage) {
		$n1 = asproMenuNormalizePath($link);
		$n2 = asproMenuNormalizePath($curPage);
		return $n1 !== '' && $n1 === $n2;
	}
}
?>
<?if($arResult):?>
	<?if (!function_exists('showSubItemss')) {
		function showSubItemss($arParams = [
			...
			<?endif;?>
			<?
			$bSelfLink = ($arParams['ITEM']["SELECTED"] ?? false) || asproMenuIsSelfLink($arParams['ITEM']["LINK"] ?? '', $GLOBALS['__curPage'] ?? '');
			?>
			<?if(($arParams['ITEM']["LINK"] ?? '') && !$bSelfLink):?>
			...
			<?if($arParams['ITEM']["CHILD"] && $arParams['SHOW_CHILDS']):?>
				...
				<ul class="dropdown-menu toggle_menu">
					<?foreach($arParams['ITEM']["CHILD"] as $key => $arSubSubItem):?>
						...
						<?
						$bSelfLink2 = ($arSubSubItem["SELECTED"] ?? false) || asproMenuIsSelfLink($arSubSubItem["LINK"] ?? '', $GLOBALS['__curPage'] ?? '');
						?>
						<li class="menu-item <?=(++$key > $arParams['VISIBLE_ITEMS_MENU'] ? 'collapsed' : '');?> <?=($arSubSubItem["CHILD"] && $arParams['SHOW_CHILDS'] ? "dropdown-submenu" : "")?> <?=($arSubSubItem["SELECTED"] ? "active" : "")?>">
							...
							<?else:?>
								<span class="name"><?=$arSubSubItem["TEXT"]?></span>
							<?endif;?>
							<?if($arSubSubItem["CHILD"] && $arParams['SHOW_CHILDS']):?>
								<ul class="dropdown-menu">
									<?foreach($arSubSubItem["CHILD"] as $arSubSubSubItem):?>
										<?
										$bSelfLink3 = ($arSubSubSubItem["SELECTED"] ?? false) || asproMenuIsSelfLink($arSubSubSubItem["LINK"] ?? '', $GLOBALS['__curPage'] ?? '');
										?>
			...
			<tr>
				<?foreach($arResult as $arItem):?>
					...
					<td class="menu-item unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							<?
							$bSelfLinkTop = ($arItem["SELECTED"] ?? false) || asproMenuIsSelfLink($arItem["LINK"] ?? '', $__curPage);
							?>
							<?if(!$bSelfLinkTop):?>
								<a class="<?=($arItem["CHILD"] && $bShowChilds ? "dropdown-toggle" : "")?>" <?=($arItem["LINK"]=='https://doctortafi.ru/' ? "rel='nofollow'" : "")?> 
								<?if($arItem["PARAMS"]["TARGET"]){?>target="<?=$arItem["PARAMS"]["TARGET"]?>"<?}?> 
								href="<?=$arItem["LINK"]?>">
							<?else:?>
								<span class="<?=($arItem["CHILD"] && $bShowChilds ? "dropdown-toggle" : "")?>" <?=($arItem["LINK"]=='https://doctortafi.ru/' ? "rel='nofollow'" : "")?>>
							<?endif;?>
								<div>
												...
												<div class="customScrollbar">
													<div class="menu-navigation__sections">
														<?foreach($arItem["CHILD"] as $arChild):?>
															<?
															$bSelfLinkChild = ($arChild["SELECTED"] ?? false) || asproMenuIsSelfLink($arChild["LINK"] ?? '', $__curPage);
															?>
											...
											<?foreach($arItem["CHILD"] as $arSubItem):?>
												<?if($MENU_TYPE == 2 && $bWideMenu):?>
													...
													<li class="<?=($arSubItem["SELECTED"] ? "active" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($bHasPicture ? "has_img" : "")?>">
														...
														<?endif;?>
														<?
														$bSelfLinkSubItem = ($arSubItem["SELECTED"] ?? false) || asproMenuIsSelfLink($arSubItem["LINK"] ?? '', $__curPage);
														?>
														<?if(!$bSelfLinkSubItem):?>
