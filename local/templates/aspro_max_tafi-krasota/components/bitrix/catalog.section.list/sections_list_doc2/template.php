...
<?
		...
		//		print_r($arItems);
							...
							//$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'CODE' => $arItems["CODE"], 'SITE_ID' => "s1"));
							//$section = $res->Fetch();
							//echo $section["ID"];

/* $exp0 = explode("?",$APPLICATION->GetCurPage());
$exp = explode("/",$exp0[0]);
							//echo count($exp); // 5 // 6
							//echo $arSection[SECTION_PAGE_URL]." - ".$exp[5];
							
							

if (count($exp) == 5) {
	$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'CODE' => $exp[3], 'SITE_ID' => "s1"));
	$section = $res->GetNext();
}
if (count($exp) == 6) {
	$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'CODE' => $exp[4], 'SITE_ID' => "s1"));
	$section = $res->GetNext();
}
if (count($exp) == 7) {
	$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'CODE' => $exp[4], 'SITE_ID' => "s1"));
...
/* if (count($exp) == 6) {
?>
<a href="/company/docs/<?=$exp[3]?>/<?=$exp[4]?>/" class="dark_link" style="font-weight: bold;"><span class="<?=($bSlideBlock ? 'font_mlg' : 'font_md');?>"><?=$section["NAME"]?></span></a>
<br><br>
<?
...
<?/* if($arResult['SECTION']){?>
	...
	<a href="<?=$arResult['SECTION']["SECTION_PAGE_URL"]?>" class="dark_link" style="font-weight: bold;"><span class="<?=($bSlideBlock ? 'font_mlg' : 'font_md');?>"><?=$arResult['SECTION']["NAME"]?></span></a>
<?} */?>


<?
$cp = $this->__component;
$arResult['ACTIVE_SECTION'] = $arResult['SECTION'];
$cp->SetResultCacheKeys(array("ACTIVE_SECTION"));
...
<?//echo "<pre>"; var_dump($arResult["SECTIONS"]); echo "</pre>";?>
	...
	<div class="catalog_section_list row items<?=($bSlick ? ' margin0' : '');?> flexbox type_<?=$arParams['TEMPLATE_TYPE']?>">
		<?foreach( $arResult["SECTIONS"] as $arItems ){
			...
			/* if($arItems["RELATIVE_DEPTH_LEVEL"] > 1){
				continue;
			} */
...
<?
	//echo $arItems["SECTION_PAGE_URL"];
	//echo "/company/docs/".$exp[3]."/".$exp[4]."/";
	//echo count($exp);
/* if ((count($exp) != 6 && $exp[5] != "") ||count($exp) == 5 || count($exp) == 4 || count($exp) != 6) { */

	/* if(count($exp) != 7) {

	if (($arItems["SECTION_PAGE_URL"] == "/company/docs/".$exp[3]."/") || ($APPLICATION->GetCurPage() == "/company/docs/")) { */
?>
...
?>
				...
				<div class="section_item item bordered box-shadow">
					<table class="section_item_inner">
						<tr>
