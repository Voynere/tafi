	...
	<?else:?>
		...
		<?else:?>
			<div class="form">
									...
									<?switch ($FIELD){
										...
										default:?>
											...
											<input 
												size="30" 
												id="input_<?=$FIELD;?>" 
												<?=(($FIELD == "LOGIN" && $arTheme["LOGIN_EQUAL_EMAIL"]["VALUE"] == "Y") ? 'type="hidden" id="input_login" value="'.$arResult["VALUES"]["EMAIL"].'"' : 'type="text"');?> 
												name="REGISTER[<?=$FIELD?>]" 
				...
				$(document).ready(function(){
					...
					$('#input_EMAIL').on('input', function() {
						...
						$('#registraion-page-form').find('#input_LOGIN').val(emailValue); 
					})
					...
					$('#registraion-page-form').submit(function(){
						...
						$(this).find('input_login').val(mail);
					...
					// console.log(arMaxOptions['THEME'])

					// .form-control #input_LAST_NAME
					// .form-control #input_NAME
					// .form-control #input_SECOND_NAME
					// <label id="input_NAME-error" class="error" for="input_NAME">Заполните это поле</label>

          function addErrorText(id, value) {
            return `<label id="${id}-errors" class="error">${value}</label>`
          }

          function ErrorValid() {
            if ($(".form-control label.error:not(for)").length != 0) {
              $(".form_footer button").attr("disabled", "disabled")
            } else {
              $(".form_footer button").removeAttr("disabled")
            }
          }
          ...
          function test() {
            ...
            var input = $(this);
            var inputContainer = $(this).closest(".form-control");

            if (/[^а-яА-ЯёЁ]/.test(inputValue) || /\d/.test(inputValue) || /\s/.test(inputValue)) {
              input.val(inputValue.replace(/[^а-яА-ЯёЁ]/g, ''));

              // inputContainer.addClass("error");
              // if (inputContainer.find("label.error").length == 0 || inputContainer.find("label.error:not(for)").length == 0) {
                inputContainer.append(addErrorText(input.attr("id"), 'Только русские буквы без пробелов и цифр'))
              // }
            }
            else {
          ...
          $(".form-control #input_LAST_NAME, .form-control #input_NAME, .form-control #input_SECOND_NAME").on("input", test);
          test()
				});
				</script>
			</div>
