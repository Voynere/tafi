		...
		<div class="form inline <?= $arResult["arForm"]["SID"] ?>">
			...
			<? if (strlen($arResult["FORM_NOTE"])) { ?>
				...
				<div class="popup form_14">
					<a href="#" class="close jqmClose"><i class="svg inline  svg-inline-" aria-hidden="true"><svg
					...
					<div class="form_result <?= ($arResult["isFormErrors"] == "Y" ? 'error' : 'success') ?>">
						...
						<? else: ?>
							...
							<script>
								...
								$('.popup.form_14 .close').click(function(){
									$('.popup.form_14').hide();
									$('.jqmOverlay').hide();
								})
			...
			<? if ($arParams['HIDE_SUCCESS'] != 'Y' || ($arParams['HIDE_SUCCESS'] == 'Y' && !strlen($arResult["FORM_NOTE"]))): ?>
				...
				<div class="form_footer long">
					...
					<? endif; ?>
					<button type="submit" class="btn btn-default long"
						value="submit"><span><?= $arResult["arForm"]["BUTTON"] ?></span></button>
					<input type="hidden" class="btn btn-default" value="<?= $arResult["arForm"]["BUTTON"] ?>"
						name="web_form_submit">
					<!--			<button type="reset" class="btn btn-default white" value="reset" name="web_form_reset" ><span>--><?php //=GetMessage('FORM_RESET') ?><!--</span></button>-->
					<script type="text/javascript">
						$(document).ready(function () {
							...
							if (arMaxOptions['THEME']['PHONE_MASK'].length) {
								...
								$('form[name=<?= $arResult["arForm"]["VARNAME"] ?>] input.phone, form[name=<?= $arResult["arForm"]["VARNAME"] ?>] input[data-sid=SIMPLE_QUESTION_457]').inputmask('mask', { 'mask': arMaxOptions['THEME']['PHONE_MASK'] });
								$('form[name=<?= $arResult["arForm"]["VARNAME"] ?>]  form[name=<?= $arResult["arForm"]["VARNAME"] ?>] input[data-sid="SIMPLE_QUESTION_123"]').inputmask('mask', { 'mask': arMaxOptions['THEME']['PHONE_MASK'] });
...
<script>
	$('[data-sid="SIMPLE_QUESTION_123"]').inputmask("+7 (999) 999 99 99");
