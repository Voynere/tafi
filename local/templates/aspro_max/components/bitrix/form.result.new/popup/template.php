...
<?php
	$reqParams = json_decode($_REQUEST['data-trigger'], associative: true);
?>
		...
		<div class="form_body">
			<?if(is_array($arResult["QUESTIONS"])):?>
				<?foreach($arResult["QUESTIONS"] as $FIELD_SID => &$arQuestion):?>
					<?php 
						$specKey = $reqParams['data-param-' . mb_strtolower($FIELD_SID, 'UTF-8')];
						if ($specKey)
						{
							$arResult['INPUT_POST_HANDLER']['ID'] = $FIELD_SID;
							$arResult['INPUT_POST_HANDLER']['NAME'] = str_replace('@', ' ', $specKey);
						}
					?>
...
<?php
$this->__component->SetResultCacheKeys(['INPUT_POST_HANDLER', 'arForm']);
?>