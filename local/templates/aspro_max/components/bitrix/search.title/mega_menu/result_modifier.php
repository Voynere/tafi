...
// Очистка только для всплывающих подсказок
foreach($arResult["CATEGORIES"] as &$arCategory) {
    foreach($arCategory["ITEMS"] as &$arItem) {
        $arItem["~ORIGINAL_NAME"] = $arItem["NAME"]; // Сохраняем оригинал
    }
}
...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	while($arElement = $rsElements->Fetch())
	{
		...
		if($arElement['IBLOCK_SECTION_ID']){
			$sectionsIds[$arElement['IBLOCK_SECTION_ID']] = $arElement['IBLOCK_SECTION_ID'];	
		}
		
		if($arElement["IBLOCK_ID"]){
			$iblocksIds[] = $arElement["IBLOCK_ID"];
		}
	 ...
	 if($sectionsIds){
		...
		foreach($iblocksIds as $iblockId){
			...
			$arFilterSect = array('ID' => $sectionsIds, 'IBLOCK_ID' => $iblockId);

			$rsSections = CIBlockSection::GetList(array(), $arFilterSect, false, array('ID', 'NAME', 'IBLOCK_ID', 'SECTION_PAGE_URL'));
...
$arResult['POPULAR_SERVICES'] = array();
$arResult['DOCTORS'] = array();

\Bitrix\Main\Loader::includeModule("highloadblock");

$cacheTime = 3600; 
$hlblockName = "PopularServicesSearch";

// Получение ID highload-блока с кэшированием
$query = \Bitrix\Highloadblock\HighloadBlockTable::query()
    ->setFilter(['=NAME' => $hlblockName])
    ->setSelect(['ID'])
    ->setCacheTtl($cacheTime);
...
if ($row) {
    ...
    // Получение данных highload-блока с кэшированием
    $cache = \Bitrix\Main\Application::getInstance()->getManagedCache();
    $cacheKey = "hlblock_data_{$HLBLOCK_ID}_search_popular";
    
    if ($cache->read($cacheTime, $cacheKey)) {
        ...
        $entityQuery = $entityDataClass::query()
            ->setSelect(['UF_NAME', 'UF_LINK'])
            ->setOrder(['ID' => 'ASC'])
            ->setFilter(['UF_ACTIVE' => 1])
            ->setLimit(3);
        
        $entityResult = $entityQuery->exec();
        $data = [];
        
        while ($arRow = $entityResult->fetch()) {
            $data[] = $arRow;            
        }
	...
	$arResult['POPULAR_SERVICES'] = $data;
}

// Сначала получаем все значения списка POSITION с кэшированием
$positionValuesMap = [];
...
while ($enum = $enumResult->fetch()) {
    $positionValuesMap[$enum['ID']] = $enum['VALUE'];
}

// Затем выполняем основной запрос
$elementsQuery = \Bitrix\Iblock\Elements\ElementDoctorsTable::query()
    ->setSelect([
        'ID',
        'IBLOCK_ID',
        'NAME',
...
while ($element = $elementsResult->fetchObject()) {
    ...
    $positions = [];
    $positionProperty = $element->getPropPosition();

    if ($positionProperty && !$positionProperty->isEmpty()) {
        foreach ($positionProperty->getAll() as $positionItem) {
            $value = $positionItem->getValue();
            if ($value && isset($positionValuesMap[$value])) {
                $positions[] = $positionValuesMap[$value];
            }
        }
    }
    ...
    $arResult['DOCTORS'][] = [
        ...
        'PREVIEW_PICTURE' => CFile::ResizeImageGet($element->getPreviewPicture(), array('width'=>60, 'height'=>60), BX_RESIZE_IMAGE_PROPORTIONAL),
        'POSITION' => $positionString,
        'LINK' => CIBlock::ReplaceDetailUrl($element->getIblock()->get('DETAIL_PAGE_URL'), $arItem, false, 'E')
    ];
}