...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	if(\Bitrix\Main\Loader::includeModule('itb.multidomains')){
		...
		$arParams["UF_PRICE_CODE"] =  $city['SETTINGS']["UF_PRICE_CODE"];
	...
	while($arElement = $rsElements->Fetch())
	{
		...
		$arCityProps = array();
		$rsPropCity = CIBlockElement::GetProperty($arElement["IBLOCK_ID"], $arElement["ID"], array("sort" => "asc"), Array("CODE"=>"HIDE_CITY"));
		while($arPropCity = $rsPropCity->Fetch())
		{
			if($arPropCity['VALUE'])
				$arCityProps[] = $arPropCity['VALUE'];
		}
		
		
		
		if($arCityProps && $city["SETTINGS"]["UF_XML_ID"])
		{
			if(!in_array($city["SETTINGS"]["UF_XML_ID"], $arCityProps))
			{
				$arDeleteIDs[$arElement["ID"]] = $arElement["ID"];
				unset($arResult["ELEMENTS"][$arElement["ID"]]);
				continue;
