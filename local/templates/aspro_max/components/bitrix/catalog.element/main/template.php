...
$products_in_cart = CSaleBasket::GetList(
    ...
    false, 
    false, 
    array('ID', 'PRODUCT_ID', 'NAME', 'PRODUCT_PRICE_ID', 'PRICE', 'CURRENCY', 'QUANTITY', 'DETAIL_PAGE_URL')
);

foreach($products_in_cart->arResult as $ids) {
	$res = CIBlockElement::GetProperty(26, $ids["PRODUCT_ID"], array("sort" => "asc"), Array());
            while ($ob = $res->GetNext()) {
				if ($ob["CODE"] == "TO_BASKET_TO" && $ob["VALUE"] != "") { 
					$idsArr [] = $ids["PRODUCT_ID"]; 
					$arrValueProp[] = $ob["VALUE"];
				}
            }
}

foreach($arrValueProp as $valProps) {
	if (!in_array($valProps, $idsArr, true)) {
		echo "YES";
	}
}
...
if (empty($products_in_cart->arResult))
{
echo "cart is ampty";
}
				...
				<div class="product-detail-gallery__container<?= ($bVertical ? ' product-detail-gallery__container--vertical' : ''); ?>">
					...
					<div class="product-detail-gallery__thmb-container text-center">
						<div class="product-detail-gallery__thmb-inner">
							<? if (!$showCustomOffer || empty($arResult['OFFERS_PROP'])) : ?>
								<? if ($countPhoto  > 1) : ?>
									<div class="product-detail-gallery__slider owl-carousel owl-theme thmb<?= ($bVertical ? ' product-detail-gallery__slider--vertical' : ''); ?>" data-size="<?= $countPhoto; ?>" data-plugin-options='{"items": "4", "nav": true, "loop": false, "clickTo": ".product-detail-gallery__slider.big", "dots": false, "autoWidth": true, "margin": 10<? //if($bVertical):
		...
		<div class="right_info">
			<div class="info_item hidden-md hidden-lg">
				...
				<? if ($arResult['HELP_TEXT']) : ?>
					<? ob_start() ?>
					<div class="text-form">
						<div class="price_txt muted777 font_sxs muted ncolor" style="position:relative">
							<?= CMax::showIconSvg("info_big pull-left", SITE_TEMPLATE_PATH . '/images/svg/catalog/watch-icon.svg', '', '', true, false); ?>
							<div class="text-form-info">
                                <span style="font-weight: 700;">Срок выполнения: </span>
                            ...
                            <div class="text-form-info__popup" >
                                <p>
                                    Срок выполнения исследования исчисляется с момента поступления в лабораторию.
                                </p>
                            </div>
						</div>
					</div>
						...
						<div class="buy_block">
							...
							<? if (!$arResult["OFFERS"]) : ?>
								<div class="counter_wrapp list big clearfix ">
									...
									<div id="<? echo $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block <?= ($arResult['CATALOG_QUANTITY'] == 0 ? "button_block_inactive" : "") ?> <?= (($arAddToBasketData["ACTION"] == "ORDER" /*&& !$arResult["CAN_BUY"]*/) || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || ($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y")  ? "wide" : ""); ?>">
										<!--noindex-->
								...
								<? if ($arResult['CATALOG_QUANTITY'] == 0) { ?>
									<div class="not_available_product"><?= GetMessage("PRODUCT_NOT_AVAILABLE"); ?></div>
								<? } ?>
...
<?
if ($arResult["PROPERTIES"]["BOOKING_LINK"]["VALUE"]) {
	?>
	<script>
			$(document).ready(function() {
				$('.not_available_product').hide();
				$('.button_block.wide').removeClass('button_block_inactive');
				$('.button_block.wide').each(function() {
					$(this).html('<a href="/booking/" class="btn-lg btn btn-default transition_bg">Записаться онлайн</a>');
				});
			});
	</script>
	<?
