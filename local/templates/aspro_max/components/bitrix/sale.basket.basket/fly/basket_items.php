...
if ($normalCount > 0) :
	...
	<?
	$arBasketItemsNotDelete = [];

	foreach ($arResult["GRID"]["ROWS"] as $k => $arItem) {
		$resProps = CIBlockElement::GetProperty(26, $arItem["PRODUCT_ID"], array("sort" => "asc"), array());
		while ($obProps = $resProps->GetNext()) {
			if ($obProps["CODE"] == "TO_BASKET_TO" && $obProps["VALUE"] != "") {
				$arBasketItemsNotDelete[] = $obProps['VALUE'];
			}
		}
	}
	?>
	...
	<div class="basket_wrap">
							...
							<div class="wrap clearfix">
								...
								*/ ?>
								<div class="body-info">
									...
									<div class="bottom">
										...
										<div class="buy_block">
											<div class="counter_block basket">
												...
												<input type="text" class="text" id="QUANTITY_INPUT_<?= $arItem["ID"] ?>" name="QUANTITY_INPUT_<?= $arItem["ID"] ?>" size="2" data-id="<?= $arItem["ID"]; ?>" data-float_ratio="<?= $float_ratio; ?>" maxlength="18" min="0" <?= $max ?> step="<?= $ratio ?>" value="<?= $arItem["QUANTITY"] ?>" onchange="updateQuantityFly('QUANTITY_INPUT_<?= $arItem["ID"] ?>', '<?= $arItem["ID"] ?>', '<?= $ratio ?>')">
											...
											<? if ($arItem["QUANTITY"] > $arItem["AVAILABLE_QUANTITY"]) : ?><div class="error"><? //=GetMessage("NO_NEED_AMMOUNT")
																															?></div><? endif; ?>
										</div>
										<div class="summ">
									...
									<? if ($bDeleteColumn && !in_array($arItem['PRODUCT_ID'], $arBasketItemsNotDelete)) : ?>
		...
		<div class="buttons clearfix">
			...
			<? if (!$arError["ERROR"]) : ?>
				...
				<? else : ?>
					<div class="wrap_button pull-right 222">
						<a href="<?= $arParams["PATH_TO_BASKET"] ?>" class="btn btn-transparent-border-color btn-lg"><span>Перейти к оформлению<?///= GetMessage("GO_TO_BASKET") ?></span></a>
