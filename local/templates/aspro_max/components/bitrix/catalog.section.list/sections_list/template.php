						...
						<tr>
							...
							<td class="section_info toggle">
								...
								<ul>
									...
									<?if($arItems["SECTION_PAGE_URL"] == "/catalog/check_up_kompleksnye_programmy/"){
															$arItems["SECTION_PAGE_URL"] = '/landings/checkup/';
														}else{
									...
									<?if($arItems["SECTIONS"]):?>
										...
										<?foreach($arItems["SECTIONS"] as $key => $arItem):?>
											<li class="<?=$arItem["SECTION_PAGE_URL"] == '/catalog/analizy_po_tipu/check_up_kompleksnye_programmy/' ? "hidden" : "" ?> sect<?=(++$i > $iVisibleItemsMenu ? ' collapsed' : '');?><?=($bSlideBlock ? ' col-sm-6 font_sm' : ' font_xs');?> "><a  href="<?=$arItem["SECTION_PAGE_URL"]?>" class="<?=($bSmallBlock ? 'muted777' : '')?>"><?=$arItem["NAME"]?><?=($arItem["ELEMENT_CNT"]?'&nbsp;<span class="'.($bSmallBlock ? 'dark-color' : '').'">'.$arItem["ELEMENT_CNT"].'</span>':'');?></a></li>
										<?endforeach;?>
