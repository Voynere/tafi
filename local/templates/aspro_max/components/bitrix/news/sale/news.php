...
<? else : ?>
	...
	<? $arItems = CMaxCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), $arItemFilter, false, false, array('ID', 'NAME', 'ACTIVE_FROM', 'ARHIVE', 'ACTIVE_TO'));
	$arYears = array();
	if ($arItems) {
			...
			if ($arParams['USE_FILTER'] != 'N') {
				...
				<div class="select_head_wrap">
					...
					<div class="head-block top bordered-block rounded3 clearfix srollbar-custom">
						<div class="item-link font_upper_md  <?= ($arhive ? '' : 'active'); ?>">
							<div class="title">
								<? if ($arhive) : ?>
							...
							<div class="item-link font_upper_md <?= ($arhive ? 'active' : ''); ?>">
								<div class="title btn-inline darken">
									<? if ($arhive) : ?>
										<span class="btn-inline darken"><?= GetMessage('ARHIVE'); ?></span>
									<? else : ?>
										<a class="btn-inline dark_link" href="<?= $APPLICATION->GetCurPageParam('year=' . "arhive"); ?>"><?= GetMessage('ARHIVE'); ?></a>
									<? endif; ?>
								</div>
							</div>
