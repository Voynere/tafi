...
<?if($arResult['ITEMS']):?>
    <h2><?=$arParams['TITLE_BLOCK_SUB']?></h2>
	...
	<?endif;?>
		...
		?>
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				$imageSrc = ($arItem['PREVIEW_PICTURE'] ? $img : SITE_TEMPLATE_PATH.'/images/svg/noimage_content.svg');
				// show  date period
				$bDate = strlen($arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE']) || ($arItem['DISPLAY__FROM'] && in_array('DATE__FROM', $arParams['FIELD_CODE']));
				$bDiscountCounter = ($arItem['_TO'] && in_array('_TO', $arParams['FIELD_CODE']));
				?>
				<div class="item-wrapper col-lg-<?=$col_lg;?> col-md-<?=$col_md;?> col-sm-6 col-xs-6 col-xxs-12 clearfix <?=($bSlider ? ' item-width-261' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					...
					<div class="item box-shadow rounded3 darken-bg-animate lazy<?=$position;?>" <?=($bImage ? 'data-src="'.$imageSrc.'"' : '');?> style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>)">
						...
						<?endif;?>
						<?if($bDetailLink && $arItem['ACTIVE'] == 'Y'):?>
					...
					<?endif;?>
						<div class="inner-text">
							<?// date  period?>
							<?if($bDate):?>
								<div class="period-block<?=(!$bFonImg ? ' muted ncolor' : '');?> font_xs <?=($arItem['_TO'] ? 'red' : '');?>">
									...
									<?else:?>
										<span class="date"><?=$arItem['DISPLAY__FROM']?></span>
									<?endif;?>
								</div>
							<?endif;?>

							<div class="title title-not-link <?=($bFonImg ? 'font_mlg' : '');//'font_md'?>">
								<?if($bDetailLink && $arItem['ACTIVE'] == 'Y'):?>
									<a href="<?=$arItem['DETAIL_PAGE_URL']?>">
								<?endif;?>
								<?=$arItem['NAME'];?>
