...
$goSec = 0;
if (count($exp) == 5) {
	if(strripos("".$arSection["SECTION_PAGE_URL"]."","".$exp[3]."")) {$goSec = 1;}
}
if (count($exp) == 6) {
	if(strripos("".$arSection["SECTION_PAGE_URL"]."","".$exp[4]."")) {$goSec = 1;}
}
if (count($exp) == 7) {
	if(strripos("".$arSection["SECTION_PAGE_URL"]."","".$exp[4]."")) {$goSec = 1;}
}

if ($goSec == 1) {$tClass = "";} else {$tClass = "";}
?>

						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="<?=$tClass?> <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">

<?
//echo "<pre>";
//print_r($arSection); //[SECTION_PAGE_URL]

$exp0 = explode("?",$APPLICATION->GetCurPage());
$exp = explode("/",$exp0[0]);
$ext = explode("/",$arSection["SECTION_PAGE_URL"]);


//echo count($exp); // 5 // 6
//echo $arSection[SECTION_PAGE_URL]." - ".$exp[3]. " = ". $ext[3];
$goSec = 0;
if (count($exp) == 5) {
	if($ext[3] == $exp[3]) {$goSec = 1;}
}
if (count($exp) == 6) {
	if($ext[4] == $exp[4]) {$goSec = 1;}
}
if (count($exp) == 7) {
	if($ext[5] == $exp[5]) {$goSec = 1;}
}
...
<?
	...
	/*
$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'CODE' => $arSection["CODE"], 'SITE_ID' => "s1"));
$section = $res->Fetch();
...
//echo count($exp); // 5 // 6
//echo $arSection[SECTION_PAGE_URL]." - ".$exp[4]. " = ". $ext[4];
$goSec = 0;
if (count($exp) == 5) {
	if($ext[3] == $exp[3]) {$goSec = 1;}
}
