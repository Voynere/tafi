...
foreach ($arResult['ITEMS'] as $key => $arItem) {
	...
	if ($priceCode) {
		$priceNew = $arItem['PROPERTIES'][$priceCode]['VALUE'];
		$productId = $arItem['ID'];
		$arResult['ITEMS'][$key]["PRICES"] = \Itb\MultiDomains\Site::getPrice($productId, $arItem["PRICES"], $priceNew);
	}
}
...
if (!empty($arResult['ITEMS'])) {
	...
	foreach ($arResult['ITEMS'] as $key => $arItem) {
		...
		$arItem['CATALOG_QUANTITY'] = (0 < $arItem['CATALOG_QUANTITY'] && is_float($arItem['CATALOG_MEASURE_RATIO'])
