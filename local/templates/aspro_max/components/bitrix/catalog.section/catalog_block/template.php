...
<?if($arResult["ITEMS"]):?>
	...
	<?endif;?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			<div class="col-lg-<?=$col;?><?if($bBigBlock):?> col-md-8 col-sm-12 col-xs-12<?else:?> col-md-4 col-sm-6 col-xs-6<?endif;?> col-xxs-12 item item-parent <?=($arItem['CATALOG_QUANTITY'] == 0 ? 'item_noavailable' : '')?> item_block<?=($bBigBlock ? ' big' : '');?> <?=($arParams['SET_LINE_ELEMENT_COUNT'] ? 'custom-line' : '');?>">
					...
					<div class="inner_wrap <?=$arParams["TYPE_VIEW_BASKET_BTN"]?>">
						...
						<?else:?>
							<div class="item_info">
								...
								?>
								<?if($pre_text):?>
								<div class="preview_text muted777 font_xs"><?=strip_tags(TruncateText($pre_text,155));?></div>
								<? endif;?>
								<?=$itemSaBlock?>
								<?=$itemPrice?>
							</div>
