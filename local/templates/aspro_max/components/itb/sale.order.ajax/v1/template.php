...
if (strlen($request->get('ORDER_ID')) > 0) {
	...
	if ($arParams['HIDE_IS_DELIVERY_ONE'] == 'Y') {
		$countDeliverys = is_array($arResult['DELIVERY']) ? count($arResult['DELIVERY']) : 0;
		$hideDelivery = $countDeliverys <= 1;
	}
		...
		<div id="bx-soa-order" class="row orderform--v1 bx-<?= $arParams['TEMPLATE_THEME'] ?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<? if ($arParams['BASKET_POSITION'] === 'before'): ?>
					<!--	BASKET ITEMS BLOCK	-->
					<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active 1">
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
							...
							<? if ($arParams['PRODUCT_COLUMNS_HIDDEN']): ?>
								<div class="col-xs-12 col-sm-3 text-right"><a href="javascript:void(0)"
										class="bx-soa-editstep"><?= $arParams['MESS_MORE_DETAILS'] ?></a></div>
							<? endif; ?>
						</div>
						<!-- <div class="bx-soa-section-content container-fluid"></div> -->
						<? $APPLICATION->IncludeComponent(
							...
							array(
								...
								"PATH_TO_ORDER" => $city["SITE_DIR"] . "order/",
								"HIDE_COUPON" => "N",
				...
				<div class="pandd">
					...
					<? else: ?>
						...
						<div id="bx-soa-delivery" data-visited="false"
							class="bx-soa-section bx-active <?= $hideDelivery ? 'hide' : '' ?>">
							<div class="bx-soa-section-title-container">
								<h2 class="bx-soa-section-title col-sm-9">
									<span class="bx-soa-section-title-count"></span><?= $arParams['MESS_DELIVERY_BLOCK_NAME'] ?>
								</h2>
					...
					<? if ($arParams['SHOW_COUPONS_DELIVERY'] === 'Y' || $arParams['SHOW_COUPONS_PAY_SYSTEM'] === 'Y'): ?>
						<div id="bx-soa-coupon" data-visited="false" class="bx-soa-section bx-active">
				...
				<? if ($arParams['BASKET_POSITION'] === 'after'): ?>
					...
					<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active">
						<div class="bx-soa-section-title-container">
							...
							<button class="btn btn-lg btn-default basket-btn-checkout has-ripple"
				...
				<div id="bx-soa-orderSave">
					...
					<div class="notice-block-order">
						* Если у вас есть карта лояльности, для применения скидки предъявите её регистратору при
						оформлении.<br>
						Также просим иметь с собой паспорт и направление от врача (при его наличии).<br><br>
						Заказы обрабатываются ежедневно до 17:00. Заказ, сделанный после 17:00, будет обработан на следующий
						день.<br>
						Если вы хотите прийти в этот же день, то ваш заказ будет оформлен в филиале в обычном режиме.
					</div>
...
?>
<script>
	$('#soa-property-5').val(<?= $_COOKIE['activeCity'] ?>);
</script>