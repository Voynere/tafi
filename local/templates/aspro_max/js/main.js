// Для стандартных подсказок
$(document).on('mouseenter', '.bx_item_block', function() {
    $(this).attr('title', $(this).find('.bx_item_element span').text());
});

// Для AJAX-обновлений
BX.addCustomEvent("onSearchAjaxSuccess", function(data) {
    $('.bx_item_block').each(function() {
        $(this).attr('title', $(this).find('.bx_item_element span').text());
    });
});// Для стандартных подсказок
$(document).on('mouseenter', '.bx_item_block', function() {
					...
					if($('.bx-soa-cart-total-line-total').length && (asproShowLicence ||  asproShowOffer)){
						...
						if( (!$('.licence_block.filter').length && asproShowLicence) || (!$('.offer_block.filter').length && asproShowOffer) ){
							...
							if(!$('.licence_block.filter').length && asproShowLicence)
								$('<div class="licence_block filter label_block onoff"><label data-for="licenses_order" class="hidden error">'+BX.message('JS_REQUIRED_LICENSES')+'</label><input type="checkbox" name="licenses_order" required '+BX.Sale.OrderAjaxComponent.state_licence+' value="Y"><label data-for="licenses_order" class="license">Я согласен(а) на обработку <a href="/doc/Политика-оператора-в-отношении-обработки-ПД-МЛ.pdf" target="_blank"> персональных данных</a></label></div>').appendTo($('.license_order_wrap'));
